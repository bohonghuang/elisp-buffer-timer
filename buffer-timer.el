...
(defun buffer-timer-convert-time (ourtime)
  "Convert a high/low integer pair to a real time value."
  (+ (* (cl-first ourtime) 65536) (cl-second ourtime)))
...
(defun buffer-timer-add-to-recent-list (item)
  "Add a string to the recent transfer list."
...
(defun buffer-timer-adjust-older-time (daysago to timeamount)
  "Add TIMEAMOUNT seconds to TO for DAYSAGO in time (append .el file)."
  (interactive (list
...
(defun buffer-timer-write-el-results ()
  "Save the .el reloadable log files"
  (interactive)
  (save-excursion
    (let ((buf 
...
(defun buffer-timer-break-time (intime)
  "Take a seconds time and breaks it into a list of seconds/minutes/hours."
...
(defun buffer-timer-time-string (intime)
  "Take a seconds time and returns a formated h/m/s string."
  (let* ((tlist (buffer-timer-break-time intime))
         (seconds (cl-first tlist))
         (minutes (cl-second tlist))
         (hours (cl-third tlist)))
...
(defun buffer-timer-summarize (&optional sortby)
  ...
  (save-excursion
    ...
    (if flyspell-mode
        (flyspell-mode-off))
    (let ((list (cl-copy-list buffer-timer-data))
          (addedtime 0)
...
(defun buffer-timer-do-idle-calculations ()
  "Update the gutter display variables (buffer-timer-search-*) and saves data"
  (interactive)
  (progn
...
(defun buffer-timer-do-early-idle ()
  "Save the buffers and update the gutter
...
;; complex reporting
;;
(defvar buffer-timer-munge-map (let ((map (make-sparse-keymap "buffer-timer-munge-keys")))
                                 (define-key map [mouse-1] 'buffer-timer-toggle-munge-state)
                                 (define-key map [mouse-2] 'buffer-timer-toggle-munge-state)
                                 (define-key map [(return)] 'buffer-timer-toggle-munge-state)
                                 map)
         ...
         ((stringp (cdr rest))
          (if (string-match (cdr rest) addstring)
              (progn
                ...
                (setq ret t))))
         (t (if (setq ret (buffer-timer-add-to-master (cdr rest) addstring value 
                                                      (format "%s " indent)))
                (progn
                  (setcar (car master) (+ value currentnum)))))))
...
(defun buffer-timer-get-days-ago (num)
  ...
  (let* ((now (current-time))
         (low (+ (cl-second now) (* num 60 60 24)))
         (high (cl-first now)))
...
(defun buffer-timer-generate-munged (&optional list)
  (let* ((list (or list (cl-copy-list buffer-timer-data)))
